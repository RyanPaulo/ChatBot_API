...
def get_trabalho_academico_by_tipo_e_disciplina(tipo: str, nome_disciplina: str):
    try:
        ...
        if not disciplina_encontrada:
            raise HTTPException(
                status_code=404, detail=f"Disciplina '{nome_disciplina}' não encontrada."
            )
        ...
        # Agora buscar o trabalho acadêmico usando tipo normalizado e id_disciplina
        response = (
...
def get_trabalhos_por_tipo(tipo: str):
    """
    ...
    try:
        ...
        # Buscar todos os trabalhos acadêmicos com o tipo especificado
        response = (
            supabase.table("trabalho_academico")
            .select("*")
            .eq("tipo", tipo_normalizado)
